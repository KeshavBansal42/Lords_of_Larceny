...
SELECT 
    vb.village_id,
    COALESCE(SUM(CASE WHEN rgc.name = 'Gold Mine' AND vb.status='active' THEN rgc.production_per_min ELSE 0 END), 0) AS total_gold_rate,
    COALESCE(SUM(CASE WHEN rgc.name = 'Gold Mine' THEN rgc.capacity ELSE 0 END), 0) AS total_gold_cap,
    COALESCE(SUM(CASE WHEN rgc.name = 'Elixir Collector' AND vb.status='active' THEN rgc.production_per_min ELSE 0 END), 0) AS total_elixir_rate,
